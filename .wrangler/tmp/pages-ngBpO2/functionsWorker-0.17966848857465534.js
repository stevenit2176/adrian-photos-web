  ...
  try {
    ...
    if (photoIds.length > 0) {
      ...
      for (const cat of categoriesResult) {
        if (!categoriesMap.has(cat.photo_id)) {
          categoriesMap.set(cat.photo_id, []);
        }
        categoriesMap.get(cat.photo_id).push({
          id: cat.id,
          name: cat.name,
          slug: cat.slug
        });
...
// ../.wrangler/tmp/pages-ngBpO2/functionsRoutes-0.10888190555251165.mjs
var routes = [
  {
